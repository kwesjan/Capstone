...
if(isset($_POST['jobid']) && !empty($_POST['jobid'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("UPDATE tbl_jobs SET title = :title, city = :city, category = :category, type = :type, experience = :experience, department = :department, description = :description, requirements = :requirements WHERE job_id = :jobid AND company = :myid");
