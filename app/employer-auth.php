...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT * FROM tbl_employers WHERE email = :myemail AND login = :mypassword");
        ...
        if ($rec == 0) {
            ...
            header("location:../employer-login.php?r=0346");
            exit();
        }
        ...
        foreach ($result as $row) {
            ...
            if ($approval_status !== 'approved') {
                // If the approval status is not 'approved', deny login
                header("location:../employer-login.php?p=Employer&r=9111");
                exit();
            }
	...
	$_SESSION['role'] = $role;
                // Set other session variables for employee
            } else {
                $_SESSION['myid'] = $row['member_no'];
                ...
                // Set other session variables for employer
            }
            $_SESSION['myid'] = $row['member_no'];
                $_SESSION['compname'] = $row['first_name'];
                $_SESSION['established'] = $row['byear'];
	...
	$_SESSION['role'] = $role;
            ...
            $stmt = $conn->prepare("UPDATE tbl_employers SET last_login = :lastlogin WHERE email= :email");
