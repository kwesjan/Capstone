...
<head>
	...
	<title>JobLinkX - Company Profile</title>
						...
						<div class="GridLex-grid-noGutter-equalHeight">
							...
							<div class="GridLex-col-3_sm-4_xs-12">
								...
								<div class="admin-sidebar">
									...
									<li class="active">
											<a href="./" class="btn btn-success btn-sm fa fa-user">Dashboard</a>
										</li>
            ...
            <table class="admin-table">
				...
				<?php
					$page1 = 0;
						...
						try {
    						...
    						$stmt = $conn->prepare("SELECT * FROM tbl_employers WHERE role = 'employer' ORDER BY first_name LIMIT $page1,16");
    						$stmt->execute();
				...
				?>
						<tr>
							...
							<td class= "user-status"><?php echo $row['user_status']; ?></td>
							<td>
								<form id="deny_Form<?php echo $row['id']; ?>" action="dash_delemploy.php" method="POST">
									<input type="hidden" name="id" value="<?php echo $row['id']; ?>">
									<button type="button" class="bg-danger" onclick="confirmDecline(<?php echo $row['id']; ?>)">Delete</button>
								</form>

										<script>
											function confirmDecline(applicationId) {
											if (confirm('This will delete the account! Do you still want to continue?')) {
            ...
            <table class="admin-table">
				...
				<?php
					$page2 = 0;
						...
						try {
    						$conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
    						$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    						$stmt = $conn->prepare("SELECT * FROM tbl_users WHERE role = 'employee' ORDER BY first_name LIMIT $page2,16");
    						$stmt->execute();
    						$result = $stmt->fetchAll();
				...
				?>
						<tr>
							...
							<td class= "user-status"><?php echo $row['user_status']; ?></td>
							<td>
								<form id="denyForm<?php echo $row['id']; ?>" action="dash_deluser.php" method="POST">
									<input type="hidden" name="id" value="<?php echo $row['id']; ?>">
									<button type="button" class="bg-danger" onclick="confirmDecline(<?php echo $row['id']; ?>)">Delete</button>
								</form>
